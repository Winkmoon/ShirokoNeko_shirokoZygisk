...
(async () => {
  ...
  const shirokozygisk_icon_state = document.getElementById('shirokozygisk_icon_state')
  ...
  console.log('[shirokozygisk.js] Android version: ', androidVersionCmd.stdout)
  ...
  console.log('[shirokozygisk.js] Kernel version: ', unameCmd.stdout)

  const catCmd = await exec('/system/bin/cat /data/adb/shirokozygisk/module.prop')
  console.log(`[shirokozygisk.js] shirokozygisk module infomation:\n${catCmd.stdout}`)

  let expectedWorking = 0
  let actuallyWorking = 0

  const shirokozygiskInfo = {
    rootImpl: null,
    monitor: null,
    zygotes: [],
    daemons: []
  ...
  if (catCmd.errno === 0) {
    ...
    moduleInfo.match(/\(([^)]+)\)/g).forEach((area) => {
      ...
      shirokozygiskInfo.rootImpl = rootImpl
    ...
    infoArea.forEach((info) => {
      if (info.startsWith('monitor:')) {
        shirokozygiskInfo.monitor = info.substring('monitor: X '.length).trim()
      }

      if (info.startsWith('zygote')) {
        shirokozygiskInfo.zygotes.push({
      ...
      if (info.startsWith('daemon')) {
        shirokozygiskInfo.daemons.push({
          bits: info.substring('daemon'.length, 'daemon'.length + 'XX'.length),
          state: info.substring('daemonXX: X '.length).trim(),
          modules: daemonModules[shirokozygiskInfo.daemons.length] || []
        })
      }
    })

    switch (shirokozygiskInfo.monitor) {
    ...
    expectedWorking = shirokozygiskInfo.zygotes.length

    for (let i = 0; i < shirokozygiskInfo.zygotes.length; i++) {
      const zygote = shirokozygiskInfo.zygotes[i]
      /* INFO: Not used ATM */
      /* const daemon = shirokozygiskInfo.daemons[i] */
  ...
  if (expectedWorking === 0 || actuallyWorking === 0) {
    shirokozygisk_state.innerHTML = translations.page.home.status.notWorking
  } else if (expectedWorking === actuallyWorking) {
    shirokozygisk_state.innerHTML = translations.page.home.status.ok

    rootCss.style.setProperty('--bright', '#3a4857')
    shirokozygisk_icon_state.innerHTML = '<img class="brightc" src="assets/tick.svg">'
  } else {
    shirokozygisk_state.innerHTML = translations.page.home.status.partially

    rootCss.style.setProperty('--bright', '#766000')
    shirokozygisk_icon_state.innerHTML = '<img class="brightc" src="assets/warn.svg">'
  }

  if (shirokozygiskInfo.rootImpl)
    root_impl.innerHTML = shirokozygiskInfo.rootImpl

  const all_modules = []
  shirokozygiskInfo.daemons.forEach((daemon) => {
  ...
  if (all_modules.length !== 0) {
    ...
    console.log(`[shirokozygisk.js] Module list:`)
  ...
  console.log('[shirokozygisk.js] boot time: ', Date.now() - start_time, 'ms')
